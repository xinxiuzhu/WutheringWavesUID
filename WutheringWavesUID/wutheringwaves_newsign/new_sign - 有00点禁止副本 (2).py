...
# ====== 新增时间检查函数 ======
def is_manual_sign_allowed() -> bool:
    """检查当前是否允许手动签到（00:00-01:00禁止）"""
    tz = timezone(timedelta(hours=8))  # 东八区时间
    now = datetime.now(tz)
    return not (0 <= now.hour < 1)  # 0点至1点返回False
...
async def sign_up_handler(bot: Bot, ev: Event):
    """处理手动签到请求"""
    # ====== 新增时间限制 ======
    if not is_manual_sign_allowed():
        return "⏰ 每日凌晨0:00~1:00为【ww开启自动签到】时间，暂无法手动签到，01:00后自由手签"
